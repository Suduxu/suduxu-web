...
```rust
for_client(id).set_frame_rate(60);
```

<Callout type="info">
The frame rate can be set to any value between 10 and 200 FPS.
</Callout>

<Callout type="warning">
Setting the frame rate to a very high value may impact performance. Furthermore clients can decide in the settings, what their maximum frame rate is, so the command may not always succeed in setting the desired frame rate, although it will be clamped to the maximum supported by the client.
</Callout>
...
```cs
suduxu.Client.For(id).SetFrameRate(60);
```
<Callout type="info">
The frame rate can be set to any value between 10 and 200 FPS.
</Callout>
